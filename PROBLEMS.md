...
| SR-P005 | P2 | OPEN | Calendar/queue operational controls and lifecycle controls are incomplete. | The source roadmap calls for calendar month/week/day views, queue management, drag/reschedule, edit/cancel/duplicate/retry actions, filtering, and bulk operations. The current dashboard has only a basic upcoming-post list. | Implement state-safe backend lifecycle APIs, persistence transitions, authorization, queue/calendar UI, filters, bulk actions, and deterministic tests. |
...
| SR-P009 | P2 | OPEN | Multi-record scheduling creation is not transactionally atomic. | Both social scheduling and V17 WhatsApp campaign creation currently persist their parent record, child records, and scheduler job through sequential repository calls. A database failure between those writes can leave a post/campaign or recipient/publication without its corresponding executable scheduler job. This is a persistence-consistency issue, not a confirmed duplicate external-send path. | Add repository-level atomic scheduling units for PostgreSQL transactions and equivalent all-or-nothing JSON mutations; add failure-injection tests proving parent/child/job records commit or roll back together. |
