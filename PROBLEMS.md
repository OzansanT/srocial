...
| --- | --- | --- | --- | --- | --- |
| SR-P001 | P1 | PARTIAL | Real-browser E2E now covers authentication/RBAC, critical content workflows, operator surfaces, and the main Queue/Calendar lifecycle paths; only a small set of direct dialog/edge interactions remains. | V22 verified auth/RBAC and Admin user lifecycle. V23 added drafts/resources/overrides/previews, Media Library, account-bound scheduling, and Queue/Calendar bulk reschedule/cancel. V24 adds deterministic Accounts state/disconnect/sanitized OAuth-result handling, WhatsApp contact/campaign/opt-out, Operations rendering/refresh, Analytics rendering/filtering, Queue state filters/edit/duplicate/retry, and Calendar drag/drop reschedule. Exact-head V24 implementation run `34975709590` passed migrations through `009_users_roles.sql`, 447/447 deterministic Node tests, JavaScript syntax, and all 12/12 real Chrome/CDP scenarios. Individual prompt-reschedule and individual cancel dialog variants plus any remaining page-specific edge interactions are not yet directly exercised. | Add direct browser assertions for the remaining individual Queue dialog/action variants and close any edge interaction found. Keep live OAuth/provider publishing/webhook/WhatsApp delivery/analytics verification separate under provider-specific VERIFY items. |
