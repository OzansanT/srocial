...
| --- | --- | --- | --- | --- | --- |
| SR-P001 | P1 | VERIFY | Browser end-to-end interaction coverage is still missing. | Current CI verifies Node tests, PostgreSQL migrations, JavaScript syntax, and browser-module contracts, but there is no real browser/E2E suite. V19 added server-persisted draft workflows, V20 added Analytics, and V21 adds persisted multi-user authentication, four-role RBAC, session revocation, and an Admin-only Users surface. These behaviors are deterministically tested at module/API level but not exercised through an actual browser session. | Add browser-level tests for critical operator flows: login as Viewer/Editor/Manager/Admin; confirm role-appropriate navigation/actions and server 403 boundaries; Admin Users create/update/disable/password-change/session-revoke flows; account connect/reconnect/disconnect; media upload/library; V19 drafts/recovery/conflicts/resources/overrides/previews; scheduling; queue/calendar lifecycle; WhatsApp; Operations; V20 Analytics; and logout. |
