...
| --- | --- | --- | --- | --- | --- |
| SR-P001 | P1 | PARTIAL | Browser end-to-end interaction coverage exists for authentication/RBAC but is still incomplete across critical operator workflows. | V22 adds a real headless-Chrome CI gate that exercises unauthenticated redirect, Admin login/logout, Viewer/Editor/Manager/Admin UI visibility plus server authorization boundaries, Admin user creation/update/role change/password rotation/disable, and explicit session revocation. Exact-head CI run `34967247398` passed all migrations, 446/446 Node tests, syntax, and all 3 real-browser scenarios. Account management, media, V19 composer workflows, scheduling, Queue/Calendar, WhatsApp, Operations, and V20 Analytics are not yet exercised through the real browser. | Extend the browser suite across account connect/reconnect/disconnect; media upload/library; V19 drafts/autosave/recovery/conflicts/resources/overrides/previews/compatibility; scheduling and Queue/Calendar lifecycle; WhatsApp; Operations; and V20 Analytics. Keep provider-live verification separate where credentials/public callbacks are required. |
