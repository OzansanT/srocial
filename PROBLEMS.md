...
| --- | --- | --- | --- | --- | --- |
| SR-P001 | P1 | VERIFY | Browser end-to-end interaction coverage is still missing. | Current CI verifies Node tests, PostgreSQL migrations, JavaScript syntax, and browser-module contracts, but there is no real browser/E2E suite. V19 adds server-persisted draft autosave/recovery, optimistic revision conflicts, reusable caption/hashtag/destination resources, platform overrides, compatibility refresh, and previews; these interactions are deterministically tested at module/API level but not exercised through an actual browser session. | Add browser-level tests for critical operator flows such as login, account connect/reconnect/disconnect UI state, media upload/library use, V19 draft autosave + refresh recovery + stale-revision conflict handling, reusable-resource apply/delete flows, per-platform overrides, compatibility/previews, scheduling from a recovered draft, validation errors, queue/calendar lifecycle actions, WhatsApp operator workflows, Operations Center behavior, and logout. |
