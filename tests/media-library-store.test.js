...
test('lists uploaded assets and reports aggregate usage', async () => {
  await withStore(async (store) => {
    const first = await store.save(Readable.from([Buffer.from('abc')]), { contentType: 'image/jpeg' });
    const second = await store.save(Readable.from([Buffer.from('12345')]), { contentType: 'video/mp4' });
    ...
    assert.equal(usage.remainingBytes, 1016);
